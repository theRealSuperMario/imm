...
def get_marker_style(i, cmap="Dark2"):
    ...
    colors = [cmap(c) for c in np.linspace(0.0, 1.0, 8)]
    markers = ["v", "o", "s", "d", "^", "x", "+"]
...
def plot_landmark(ax, landmark, k, size=1.5, zorder=2, cmap="Dark2", style_fn=None):
    if style_fn is None:
...
def plot_landmarks(ax, landmarks, size=1.5, zorder=2, cmap="Dark2", style_fn=None):
    for k, landmark in enumerate(landmarks):
        plot_landmark(
            ax, landmark, k, size=size, zorder=zorder, cmap=cmap, style_fn=style_fn
        )
