...
def get_numeric_shape(t):
  ...
  """
    ...
    ds = [
        tf.unique(tf.reshape(tf.reduce_sum(o, reduction_indices=[i]), [-1]))[0][0]
        for i in range(o.get_shape().ndims)
    ]
    return ds
...
def dedup(t, v):
    ...
    def collapse(seq, i_s):
        ...
        res = tf.cond(tf.reduce_all(tf.equal(i, 0)), lambda: v1, lambda: dedup_val)
        return res
...
def split_tensors(ts, num_splits, axis=0):
  ...
  """
    ...
    splits = [
        nest.pack_sequence_as(ts, [s[i] for s in splits]) for i in range(num_splits)
    ]
    return splits
...
def merge_tensors(ts_split, axis=0):
  ...
  """
    ...
    ts_merged = [
        tf.concat([s[i] for i in xrange(len(ts_flat))], axis=dim) for s in ts_flat
    ]
  ...
  """
    ...
    with tf.name_scope(name, "meshgrid", args) as name:
        ...
        for i, x in enumerate(args):
            output.append(
                tf.reshape(tf.expand_dims(x, 0), (s0[:i] + (-1,) + s0[i + 1 : :]))
            )
...
def get_grid(x_range, y_range, nmajor=5, nminor=20):
  ...
  """
    h_lines = [
        np.concatenate(
            np.meshgrid(np.linspace(x_range[0], x_range[1], nminor), y), axis=0
        ).T
        for y in np.linspace(y_range[0], y_range[1], nmajor)
    ]
    v_lines = [
        np.concatenate(
            np.meshgrid(x, np.linspace(y_range[0], y_range[1], nminor)), axis=1
        )
        for x in np.linspace(x_range[0], x_range[1], nmajor)
    ]
