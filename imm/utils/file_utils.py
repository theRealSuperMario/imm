...
def split_video_into_frames(
  ...
  """
    ...
    cmd = (
        "ffmpeg -hide_banner -loglevel panic -i %s"
        + seek
        + " -q:v %d -start_number 0"
        + crop
        + resize
        + " %s"
    )
...
def extract_frames_from_video(vid_fname, frame_ids=[], frame_hw=None):
  ...
  """
    ...
    cmd = (
        "ffmpeg -loglevel panic -hide_banner -i %s -f image2pipe -vsync"
...
def get_random_name(len=32):
    return "".join(
        [random.choice(string.ascii_letters + string.digits) for _ in xrange(len)]
    )
