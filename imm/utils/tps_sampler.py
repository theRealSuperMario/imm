...
class TPSRandomSampler(nn.Module):
    ...
    def forward(self, input):
        ...
        input = F.pad(
            input, (self.h_pad, self.h_pad, self.w_pad, self.w_pad), mode="replicate"
        )
...
class TPSGridGen(nn.Module):
    ...
    """
        ...
        L = np.c_[
            Kp, np.ones((self._n_grid, 1), dtype=np.float32), self._grid
        ]  # Nx(M+3)
    ...
    """
        ...
        tfm_grid = tfm_grid.reshape((batch_size, self._grid_hw[0], self._grid_hw[1], 2))
        return tfm_grid
...
def sample_tps_w(Hc, Wc, warpsd, rotsd, scalesd, transsd):
  ...
  """
    ...
    W = warpsd[0] * np.random.randn(Nc, 2) + warpsd[1] * (mask * np.random.randn(Nc, 2))
    # affine component:
    rnd = np.random.randn
    rot = np.deg2rad(rnd() * rotsd)
    sc = 1.0 + rnd() * scalesd
    aff = [
        [transsd * rnd(), transsd * rnd()],
