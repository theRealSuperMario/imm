  ...
  """
    with tf.variable_scope(scope, default_name="BNorm", values=[x], reuse=reuse) as sc:
        ...
        if is_train:
            ...
            mu_update_op = moving_averages.assign_moving_average(
                moving_mean, mean, 0.99
            )
            var_update_op = moving_averages.assign_moving_average(
            ...
            variance = variables.model_variable(
                "moving_variance",
                shape=params_shape,
                dtype=dtype,
                initializer=tf.constant_initializer(1.0, dtype),
...
def _conv(
  ...
  """
    w = _variable_with_weight_decay(
        w_name, shape=shape, stddev=std, wd=wd, dtype=dtype, device=device
    )
...
def fc_layer(
  ...
  """
    ...
    with tf.variable_scope(
        layer_name, default_name="FCLayer", values=[x], reuse=reuse
    ) as sc:
        ...
        if batch_norm:
            y = batch_norm_layer(
                y, decay=0.9, reuse=False, is_training=opts["train_switch"]
            )
    return y
...
def conv_block(
  ...
  """
    ...
    with tf.variable_scope(
        conv_scope, default_name="ConvBlock", values=[x], reuse=share_conv
    ) as sc:
        out_c = _conv(
            x,
            shape,
    ...
    if batch_norm:
        ...
        # NOTE: specify device?
        out_b = tf.layers.batch_normalization(
            out_c, training=opts["training_pl"], fused=True
        )
        out = out_b
    if layer_norm:
        out_b = tf.contrib.layers.layer_norm(
            out_c, variables_collections=tf.GraphKeys.MODEL_VARIABLES
        )
        out = out_b
    # relu:
