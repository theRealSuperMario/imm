...
def create_reset_metric(
    metric_fn, scope="reset_metric", reset_collections=None, **metric_kwargs
):
    with tf.variable_scope(None, default_name=scope):
        metric_op, update_op = metric_fn(**metric_kwargs)
        variables = tf.get_collection(
            tf.GraphKeys.LOCAL_VARIABLES, scope=tf.contrib.framework.get_name_scope()
        )
