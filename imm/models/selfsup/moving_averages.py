    ...
    with ops.op_scope([variable, value, decay], name, "AssignMovingAvg") as scope:
        with ops.colocate_with(variable):
            ...
            return state_ops.assign_sub(
                variable, (variable - value) * decay, name=scope
            )
...
def weighted_moving_average(
  ...
  """
    ...
    with variable_scope.variable_op_scope(
        [value, weight, decay], name, "WeightedMovingAvg"
    ) as scope:
...
class ExponentialMovingAverageExtended(object):
  ...
  *  Build a model normally but load the checkpoint files to evaluate by using
  ...
  """
    ...
    def __init__(
        self, decay, num_updates=None, value=None, name="ExponentialMovingAverage"
    ):
            ...
            with ops.control_dependencies(None):
                if isinstance(var, variables.Variable):
                    ...
                    if self._value is None:
                        ...
                        val = np.full(
                            var.get_shape().as_list(), self._value[i], dtype=np.float32
                        )
                        avg = slot_creator.create_slot(
                            var,
                            val,
