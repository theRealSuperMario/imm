...
def _pretrained_vgg_conv_weights_initializer(
    name, data, info=None, pre_adjust_batch_norm=False, prefix=""
):
    shape = None
...
def _pretrained_vgg_inner_weights_initializer(
    name, data, info=DummyDict(), pre_adjust_batch_norm=False, prefix=""
):
    shape = None
    if name in data and "0" in data[name]:
        W = data[name]["0"]
...
def vgg_conv(
    ...
    with tf.name_scope(name):
        ...
        assert b_shape is None or tuple(b_shape) == (
            f,
        ), "Incorrect bias shape for {} (file: {}, spec; {})".format(
            name, b_shape, (f,)
        )
        ...
        with tf.variable_scope(scope):
            W = tf.get_variable(
                "weights", shape, dtype=tf.float32, initializer=W_init, trainable=False
            )
            b = tf.get_variable(
                "biases", [f], dtype=tf.float32, initializer=b_init, trainable=False
            )
        ...
        if batch_norm:
            ...
            gamma = tf.Variable(
                tf.constant(1.0, shape=[f]), name="gamma", trainable=True
            )
            # ema = tf.train.ExponentialMovingAverage(decay=0.999)
            ema = ExponentialMovingAverageExtended(
                decay=0.999, value=[0.0, 1.0], num_updates=global_step
            )
...
def vgg_inner(
    ...
    with tf.name_scope(name):
        ...
        assert W_shape is None or tuple(W_shape) == tuple(shape), (
            "Incorrect weights shape for %s" % name
        )
        assert b_shape is None or tuple(b_shape) == (f,), (
            "Incorrect bias shape for %s" % name
        )
...
def build_network(
    ...
    def conv(z, ch, **kwargs):
        ...
        kwargs["pre_adjust_batch_norm"] = kwargs.get(
            "pre_adjust_batch_norm", pre_adjust_batch_norm
        )
