...
def to_caffe(
    ...
    shape=None,
    color_layer="",
    conv_fc_transitionals=None,
    info=DummyDict(),
):
    assert conv_fc_transitionals is None or name is not None
    if tfW.ndim == 4:
        if (name == "conv1_1" or name == "conv1" or name == color_layer) and tfW.shape[
            2
        ] == 3:
            tfW = tfW[:, :, ::-1]
            info[name] = "flipped"
...
def from_caffe(
    cfW, name=None, color_layer="", conv_fc_transitionals=None, info=DummyDict()
):
    assert conv_fc_transitionals is None or name is not None
    if cfW.ndim == 4:
        ...
        if key not in ignore:
            ...
            if "0" in data[key]:
                ...
                if weights is not None:
                    ...
                    loaded.append(
                        "{}:0 -> {}:weights{} {}".format(
                            key, local_key, init_str, info.get(key, "")
                        )
                    )
            ...
            if "1" in data[key]:
                ...
                if biases is not None:
                    ...
                    loaded.append(
                        "{}:1 -> {}:biases{}".format(key, local_key, init_str)
                    )
...
def save_caffemodel(
    ...
    for lay in layers:
        ...
        if weights is not None:
            ...
            d["0"] = cfW
            saved.append(
                "{}:weights -> {}:0  {}".format(prefix + p_lay, lay, info.get(lay, ""))
            )
