...
class BaseModel(object):
    ...
    def _exp_running_avg(self, x, training_pl, init_val=0.0, rho=0.99, name="x"):
        x_avg = variables.model_variable(
            name + "_agg",
            shape=x.shape,
            dtype=x.dtype,
        ...
        update_op = tf.cond(
            training_pl, lambda: tf.assign(x_avg, x_new), lambda: tf.constant(0.0)
        )
    ...
    """
        ...
        with tf.variable_scope(name, default_name="uw_mtloss") as sc:
            for i, loss in enumerate(losses):
                i_log_s = variables.model_variable(
                    "loss%d" % i,
                    shape=(1,),
                    dtype=tf.float32,
    ...
    """
        ...
        with tf.variable_scope(name, default_name="mconv") as sc:
            ...
            f_shape = [
                ...
                in_channels,
                out_channels,
            ]  # shape of the filters of the first conv-layer
            y, _ = nnu.conv_block(
                opts,
                x,
                f_shape,
