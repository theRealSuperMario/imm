...
def metrics_summary(name, metric_fn, **metric_kwargs):
    metric, _, _ = op_utils.create_reset_metric(
        metric_fn,
        updates_collections=["metrics_update"],
        reset_collections=["metrics_reset"],
        **metric_kwargs
    )
...
def colorize_landmark_maps(maps):
  ...
  """
    ...
    hmaps = [
        tf.expand_dims(maps[..., i], axis=3) * np.reshape(colors[i], [1, 1, 1, 3])
...
class IMMModel(BaseModel):
    ...
    *NOTE: Important to note that it assumes that the images are float32 tensors
    ...
    def image_encoder(self, x, training_pl, filters=64, var_device="/cpu:0"):
        """
    Image encoder
    """
    ...
    """
        with tf.variable_scope("model"):
            ...
            gauss_pt, pose_embeddings = dev_wrap(
                lambda: pose_encoder(future_im, map_sizes=render_sizes, reuse=False),
                pose_dev,
            )
    ...
    def loss(
        ...
        if self._config.reconstruction_loss == "perceptual":
            ...
            reconstruction_loss = dev_wrap(
                lambda: self._colorization_reconstruction_loss(
                    future_im, future_im_pred, training_pl, loss_mask=loss_mask
                ),
                loss_dev,
            )
            ...
            raise ValueError(
                "Reconsutruction loss-type: "
                + self._config.reconstruction_loss
                + " not understood"
            )
        self._add_cost_summary(reconstruction_loss, "reconstruction_loss")

        metrics_summary(
            "reconstruction_metric", tf.metrics.mean, values=reconstruction_loss
        )
    ...
    """
        ...
        image_encoder = lambda x: self.image_encoder(x, training_pl, filters=filters)
