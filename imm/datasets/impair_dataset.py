...
class ImagePairDataset(object):
    ...
    """
        with tf.name_scope("common_box"):
            box = tf.concat(
                [tf.minimum(box1[:2], box2[:2]), tf.maximum(box1[2:], box2[2:])], axis=0
            )
        return box
    ...
    """
        with tf.name_scope("image_jitter"):
            ...
            if swap:
                im0, im1 = tf.cond(
                    tf.random_uniform([]) < 0.5, lambda: (im0, im1), lambda: (im1, im0)
                )
        return im0, im1
    ...
    def _proc_im_pair(self, inputs, keep_aspect=True):
        with tf.name_scope("proc_im_pair"):
            ...
            should_jitter = (self._jittering is not None and self._jittering) or (
                self._jittering is None and self._subset == "train"
            )
    ...
    """
        ...
        dataset = dataset.map(
            self._proc_im_pair, num_parallel_calls=num_preprocess_threads
        )
