...
def load_dataset(data_root, dataset, subset):
    ...
    with open(
        os.path.join(data_root, "Anno", "list_landmarks_align_celeba.txt"), "r"
    ) as f:
    ...
    if dataset == "celeba":
        if subset == "train":
            ...
            raise ValueError("subset = %s for celeba dataset not recognized." % subset)
    elif dataset == "mafl":
        if subset == "train":
            label = 1
        elif subset == "test":
            ...
            raise ValueError("subset = %s for mafl dataset not recognized." % subset)
