...
def tower_loss(inputs, training_pl, model, scope):
  ...
  """
    ...
    _, loss, avg_ops = model.build(
        inputs, training_pl, costs_collection="costs", scope=scope, var_device="/cpu:0"
    )
...
def train_multi(
    opts, graph, optim, inputs, training_pl, model_factory, global_step, clip_value=None
):
    """
            ...
            for i in xrange(num_gpus):
                with tf.device("/gpu:%d" % opts["gpu_ids"][i]):
                    ...
                    with tf.name_scope("tower_%d" % i) as scope:
                        print(
                            colorize(
                                "building graph on: tower_%d" % i, "blue", bold=True
                            )
                        )
  ...
  """
    ...
    with graph.as_default(), tf.device("/cpu:0"):
        ...
        with tf.device("/gpu:%d" % opts["gpu_ids"][0]):
            ...
            apply_grad_op = optim.apply_gradients(
    ...
    with graph.as_default(), tf.device("/cpu:0"):
        ...
        with tf.name_scope("cpu_tower") as scope:
            ...
            if clip_value is not None:
                with tf.name_scope("grad_clip") as scope:
                    grads_and_vars = [
                        (tf.clip_by_norm(grad, clip_value + 0.0), var)
                        for grad, var in grads_and_vars
                    ]
...
def train_split_gpus(
    opts, graph, optim, inputs, training_pl, model_factory, global_step, clip_value
):
    """
  ...
  """
    if split_gpus:
        ...
        if num_gpus == 0:
            ...
            print(
                colorize(
                    "training on SINGLE gpu: %d" % opts["gpu_ids"][0], "red", bold=True
                )
            )
            return train_single(
                opts,
...
def train_loop(
  ...
  """
    ...
    with graph.as_default(), tf.device("/cpu:0"):
        ...
        if tf.gfile.Exists(checkpoint_fname) or tf.gfile.Exists(
            checkpoint_fname + ".index"
        ):
            print(
                colorize("RESTORING MODEL from: " + checkpoint_fname, "blue", bold=True)
            )
            ...
            if reset_global_step >= 0:
                print(
                    colorize(
                        "Setting global-step to %d." % reset_global_step,
                        "red",
                        bold=True,
                    )
                )
                ...
                reset_vid = [
                    i for i in xrange(len(var_names)) if "global_step" in var_names[i]
                ]
                if reset_vid:
            ...
            if ignore_missing_vars:
                ...
                vars_ignored = [
                    ...
                    if v.name[:-2] not in checkpoint_vars
                ]
            ...
            if exclude_vars:
                for exclude_var_name in exclude_vars:
                    ...
                    reset_vid = [
                        ...
                        if exclude_var_name in var_names[i]
                    ]
                    if reset_vid:
                        vars_to_restore.pop(reset_vid[0])
