...
def evaluate(
    ...
    with tf.Graph().as_default() as graph:
        ...
        _, loss, _, tensors = net_instance.build(
            inputs, training_pl=training_pl, output_tensors=True, build_loss=eval_loss
        )
        ...
        while True:
            try:
                ...
                if test_iter == 0 and eval_summaries:
                    results, summary_str = session.run(
                        [ops_to_run, test_summary_op], feed_dict=feed_dict
                    )
                ...
                format_str = (
                    "test: %s: step %d, loss = %.4f (%.1f examples/sec) %.3f sec/batch"
                )
                print(
                    format_str
