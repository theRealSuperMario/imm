...
def main(args):
    ...
    print(
        "error on %s datset %s set: %.5f (%.3f percent)"
        % (args.test_dataset, args.test_split, mean_error, mean_error * 100.0)
    )
    print("===========================================================")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--paths-config",
        type=str,
        default="configs/paths/default.yaml",
        required=False,
        help="Path to the paths config.",
    )
    parser.add_argument(
        "--iteration",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="Name of the buffer when using matlab data pipeline.",
    )
    parser.add_argument(
        "--im-size", type=int, default=128, required=False, help="Image size."
    )
    parser.add_argument(
        "--bias", action="store_true", required=False, help="Use bias in the regressor."
    )
    parser.add_argument(
        "--batch-size", type=int, default=100, required=False, help="batch_size"
    )
