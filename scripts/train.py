...
def main(args):
    ...
    if args.checkpoint is not None:
        ...
        print(
            colorize(
                "No checkpoint file specified. Initializing randomly.", "red", bold=True
            )
        )
    ...
    opts[
        "n_checkpoint"
    ] = train_config.ncheckpoint  # number of iteration after which to save the model
    ...
    with graph.as_default():
        ...
        if train_config.optim.lower() == "adam":
            ...
            optim = tf.train.AdadeltaOptimizer(
                lr, rho=0.95, epsilon=1e-06, use_locking=False, name="Adadelta"
            )
        ...
        factory = model_factory(IMMModel, config=config.model, global_step=global_step)
        ...
        train_dset = train_dset.get_dataset(
            batch_size, repeat=True, shuffle=False, num_preprocess_threads=12
        )
        ...
        test_dset = test_dset.get_dataset(
            batch_size, repeat=False, shuffle=False, num_preprocess_threads=12
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--configs", nargs="+", default=[], help="Paths to the config files."
    )
    parser.add_argument(
        "--ngpus",
        type=int,
    ...
    parser.add_argument(
        "--lr-multiple", type=float, default=1, help="multiplier on the learning rate."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--restore-optim", action="store_true", help="Restore the optimizer variables."
    )
    parser.add_argument(
        "--reset-global-step",
        type=int,
    ...
    parser.add_argument(
        "--ignore-missing-vars",
        action="store_true",
        help="Skip re-storing vars not in the checkpoint file.",
    )
